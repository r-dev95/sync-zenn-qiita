...
/**
 * Remove image captions and sizes.
 *
 * - `![alt](url)\n*caption*` -> `![alt](url)`
 ...
 * @returns Converted markdown
 */
const convertImgLinkToTag = (md: string): string => {
  return (
    md
      // remove image captions
      .replace(/(!\[.*?\]\(.*?\))\n\*.*?\*/g, "$1")
 ...
 * @returns Converted markdown
 */
const convertEmbeddedLink = (md: string): string => {
...
const convertCustomBlock = (md: string): string => {
  return md.replace(/:::message alert/g, ":::note alert").replace(/:::message/g, ":::note info");
};
